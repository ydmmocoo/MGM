...
public class MessageInfoUtil {
    ...
    public static final String TRANSFER_ACCOUNT_RECEIVED = "transfer_account_received";//转账
    public static final String TRANSFER_ACCOUNT_UN_RECEIVED = "transfer_account_un_received";//转账
    public static final String TRANSFER_ACCOUNT_NEW_RECEIVED = "transfer_account";//转账


    public static final String TRANSFER_RED_PACKET_RECEIVED = "transfer_red_packet_received";//红包
    public static final String TRANSFER_RED_PACKET_UN_RECEIVED = "transfer_red_packet_un_received";//红包

    public static final String TRANSFER_SHARE_RECEIVED = "transfer_share_received";//分享
    public static final String TRANSFER_SHARE_UN_RECEIVED = "transfer_share_un_received";//分享
...
//    public static final String FRIEND_ADD = "friend_add";//添加好友
    ...
    public static MessageInfo buildCustomFaceMessage(int groupId, String faceName) {
        ...
        TIMMsg.addElement(ele);
        info.setExtra("[".concat(ContextManager.getContext().getString(R.string.action_custom_emoji)).concat("]"));
        info.setMsgTime(System.currentTimeMillis());
        info.setSelf(true);
        info.setTIMMessage(TIMMsg);
        info.setFromUser(TIMManager.getInstance().getLoginUser());
    ...
    public static MessageInfo buildImageMessage(final Uri uri, boolean compressed, boolean appPohto) {
        ...
        info.setSelf(true);
        info.setTIMMessage(TIMMsg);
        info.setExtra("[".concat(ContextManager.getContext().getString(R.string.action_photo)).concat("]"));
        info.setMsgTime(System.currentTimeMillis());
        info.setFromUser(TIMManager.getInstance().getLoginUser());
    ...
    public static MessageInfo buildVideoMessage(String imgPath, String videoPath, int width, int height, long duration) {
        ...
        info.setTIMMessage(TIMMsg);
        info.setExtra("[".concat(ContextManager.getContext().getString(R.string.action_video)).concat("]"));
        info.setMsgTime(System.currentTimeMillis());
        info.setFromUser(TIMManager.getInstance().getLoginUser());
    ...
    public static MessageInfo buildAudioMessage(String recordPath, int duration) {
        ...
        info.setSelf(true);
        info.setTIMMessage(TIMMsg);
        info.setExtra("[".concat(ContextManager.getContext().getString(R.string.action_voice)).concat("]"));
        info.setMsgTime(System.currentTimeMillis());
        info.setFromUser(TIMManager.getInstance().getLoginUser());
    ...
    public static MessageInfo buildFileMessage(Uri fileUri) {
        ...
        if (file.exists()) {
            ...
            info.setSelf(true);
            info.setTIMMessage(TIMMsg);
            info.setExtra("[".concat(ContextManager.getContext().getString(R.string.action_file)).concat("]"));
            info.setMsgTime(System.currentTimeMillis());
            info.setFromUser(TIMManager.getInstance().getLoginUser());
     ...
     * @return
     */
    public static MessageInfo buildTransferCustomMessage(String action, String message) {
        MessageInfo info = new MessageInfo();
        TIMMessage TIMMsg = new TIMMessage();
//        TIMCustomElem ele = new TIMCustomElem();
//        ele.setData(action.getBytes());
...
//        TIMMsg.addElement(ele);
        ...
        info.setExtra(message);
        info.setMsgTime(System.currentTimeMillis());
        if (action.equals(TRANSFER_ACCOUNT_UN_RECEIVED)) {
            info.setMsgType(MessageInfo.MSG_TYPE_CUSTOM_TRANSFER);
        } else if (action.equals(TRANSFER_RED_PACKET_UN_RECEIVED)) {
            info.setMsgType(MessageInfo.MSG_TYPE_CUSTOM_RED_PACKET);
        } else if (TextUtils.equals(action, TRANSFER_ACCOUNT_RECEIVED)
                || TextUtils.equals(action, TRANSFER_RED_PACKET_RECEIVED)) {
            info.setMsgType(MessageInfo.MSG_TYPE_CUSTOM_TRANSFER_TIP);
        }
        return info;
    }
     ...
     * @return
     */
    public static MessageInfo buildAddFriendCustomMessage(String idtityId) {

//        {
//            "identifier" : "17359276732",
//                "messageType" : "friend_add"
//        }

        JsonObject object = new JsonObject();
        object.addProperty("identifier", idtityId);
        object.addProperty("messageType", FRIEND_ADD);

        String message = ContextManager.getContext().getString(R.string.add_friend);
        MessageInfo info = new MessageInfo();
        TIMMessage TIMMsg = new TIMMessage();
        TIMCustomElem ele = new TIMCustomElem();
        ...
        info.setExtra(message);
        info.setMsgTime(System.currentTimeMillis());
        info.setMsgType(MessageInfo.MSG_TYPE_ADD_FRIEND_TIPS);
        return info;
    }
    ...
    public static List<MessageInfo> TIMMessages2MessageInfos(List<TIMMessage> timMessages, boolean isGroup) {
        ...
        for (int i = 0; i < timMessages.size(); i++) {
            ...
            if (timMessage.getElementCount() > 0) {
                if (timMessage.getElement(0).getType() == TIMElemType.ProfileTips) continue;
            }
    ...
    public static MessageInfo timTransfer2MesssageInfo() {
        MessageInfo info = new MessageInfo();
        TIMMessage TIMMsg = new TIMMessage();
//        TIMCustomElem ele = new TIMCustomElem();
//        ele.setData(action.getBytes());
...
//        TIMMsg.addElement(ele);
    ...
    /**
     * 临时会话列表TIMMessage转MessageInfo
     * <p>用于处理会话接收消息例如收到同城、红包、转账时在外面显示信息</p>
     *
     * @param timMessage
     * @param isGroup
     * @return
        ...
        if (timMessage.getElementCount() > 0) {
            ...
            if (type == TIMElemType.Custom) {
                ...
                if (data.equals(GROUP_CREATE)) {
                    ...
                    if (model != null) {
                        if (StringUtil.equals(model.getMessageType(), ElemExtModel.SHARE_NEWS) || StringUtil.equals(model.getMessageType(), ElemExtModel.SHARE_CITY)
                                || StringUtil.equals(model.getMessageType(), ElemExtModel.SHARE_YELLOWPAGE) || StringUtil.equals(model.getMessageType(), ElemExtModel.SHARE_PERSONAL_CARD)) {
                            msgInfo.setMsgType(MessageInfo.MSG_TYPE_NEARBY_CITY);
                        } else {
                            String messageType = model.getMessageType();
                            if (TextUtils.equals(TRANSFER_ACCOUNT_UN_RECEIVED, messageType)) {
                                msgInfo.setMsgType(MessageInfo.MSG_TYPE_CUSTOM_TRANSFER);
                                msgInfo.setExtra("[收到一个转账消息]");
                            } else if (TextUtils.equals(TRANSFER_RED_PACKET_UN_RECEIVED, messageType)) {
                                msgInfo.setMsgType(MessageInfo.MSG_TYPE_CUSTOM_RED_PACKET);
                                msgInfo.setExtra("[收到一个红包]");
                            } else if (TextUtils.equals(messageType, TRANSFER_ACCOUNT_RECEIVED)) {//领取转账
                                msgInfo.setMsgType(MessageInfo.MSG_TYPE_CUSTOM_TRANSFER_TIP);
//                                msgInfo.setExtra(ContextManager.getContext().getString(R.string.transfer_receiver));
                                TIMMessage lastMsg = conversation.getLastMsg();
                                boolean self = lastMsg.isSelf();
                                if (self) {
                                    msgInfo.setExtra(ContextManager.getContext().getString(R.string.your_request_trans));
                                } else {
                                    msgInfo.setExtra(ContextManager.getContext().getString(R.string.your_response_trans));
                                }
                            } else if (TextUtils.equals(messageType, TRANSFER_ACCOUNT_NEW_RECEIVED)) {//新版领取转账
                                msgInfo.setMsgType(MessageInfo.MSG_TYPE_CUSTOM_TRANSFER_TIP);
//                                msgInfo.setExtra(ContextManager.getContext().getString(R.string.transfer_receiver));
                                TIMMessage lastMsg = conversation.getLastMsg();
                                try {
                            ...
                            } else if (TextUtils.equals(messageType, TRANSFER_RED_PACKET_RECEIVED)) {//领取红包
                                msgInfo.setMsgType(MessageInfo.MSG_TYPE_CUSTOM_TRANSFER_TIP);
//                                msgInfo.setExtra(ContextManager.getContext().getString(R.string.rp_receiver));
                                TIMMessage lastMsg = conversation.getLastMsg();
                                boolean self = lastMsg.isSelf();
                                if (self) {
                                    msgInfo.setExtra(ContextManager.getContext().getString(R.string.you_accepted_the_red_package));
                                } else {
                                    msgInfo.setExtra(ContextManager.getContext().getString(R.string.your_accepted_was_red_package));
                                }
                            } else if (TextUtils.equals(messageType, FRIEND_ADD)) {
                                msgInfo.setMsgType(MessageInfo.MSG_TYPE_ADD_FRIEND_TIPS);
                                if (TextUtils.isEmpty(new String(customElem.getExt()))) {
                                    msgInfo.setExtra("添加好友");
                                } else {
                                    msgInfo.setExtra(new String(customElem.getExt()));
                                }
                            }
...
//                    msgInfo.setExtra("[自定义消息]");
                ...
                String user = "";
                String nickName = "";
                final List<TIMUserProfile> profiles = new ArrayList<>();
                if (groupTips.getChangedGroupMemberInfo().size() > 0) {
                    Object ids[] = groupTips.getChangedGroupMemberInfo().keySet().toArray();
                    for (int i = 0; i < ids.length; i++) {
                        TIMUserProfile timUserProfile = TIMFriendshipManager.getInstance().queryUserProfile(ids[i].toString());
                        if (timUserProfile == null) {
                            List<String> idens = new ArrayList<>();
                            idens.add(ids[i].toString());
                            TIMFriendshipManager.getInstance().getUsersProfile(idens, false, new TIMValueCallBack<List<TIMUserProfile>>() {
                                @Override
                                ...
                                public void onSuccess(List<TIMUserProfile> timUserProfiles) {
                                    profiles.add(timUserProfiles.get(0));
                                }
                            });
                        } else {
                            profiles.add(timUserProfile);
                        }
                    }
                    for (int i = 0; i < profiles.size(); i++) {
                        nickName = nickName + profiles.get(i).getNickName();
                        nickName = nickName + ",";
                        if (i == 2 && ids.length > 3) {
                            nickName = nickName.substring(0, nickName.length() - 1) + "等";
                            break;
                        }
                    }

                } else {
                    String identifier = groupTips.getOpUserInfo().getIdentifier();
                    TIMUserProfile timUserProfile = TIMFriendshipManager.getInstance().queryUserProfile(identifier);
                    if (timUserProfile == null) {
                        nickName = groupTips.getOpUserInfo().getIdentifier();
                    } else {
                ...
                if (tipsType == TIMGroupTipsType.ModifyGroupInfo) {
                    ...
                    if (modifyList.size() > 0) {
                        ...
                        if (modifyType == TIMGroupTipsGroupInfoType.ModifyName) {
                            ...
                            msgInfo.setMsgType(MessageInfo.MSG_TYPE_GROUP_MODIFY_FACE_URL);
    ...
    public static String createJson(String money, String remark, String transferId, String messageType) {
        ...
        return JsonUtil.moderToString(model);
    }

    public static String createGroupRedPacketJson(String money, String remark, String rId, String messageType) {
        ElemExtModel model = new ElemExtModel();
        model.setMoney(money);
        model.setrId(rId);
        model.setRemark(remark);
