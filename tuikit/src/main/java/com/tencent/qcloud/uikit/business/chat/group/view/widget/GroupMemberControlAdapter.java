...
public class GroupMemberControlAdapter extends BaseAdapter {
    ...
    public View getView(final int i, View view, final ViewGroup viewGroup) {
        ...
        if (!TextUtils.isEmpty(info.getIconUrl())) {
            CommonImageLoader.load(info.getIconUrl())
                    .placeholder(R.drawable.default_user_image)
                    .error(R.drawable.default_user_image)
                    .into(holder.memberIcon);
        }else {
            holder.memberIcon.setImageResource(R.drawable.default_user_image);
        }
    ...
    public void setDataSource(List<GroupMemberInfo> members, boolean isWoner) {
        if (members != null) {
            if (mGroupMembers != null && mGroupMembers.size() > 0) {
                mGroupMembers.clear();
            }
            ...
            if (isWoner) {
                shootMemberCount = members.size() > 8 ? 8 : members.size();

            } else {
                shootMemberCount = members.size() > 9 ? 9 : members.size();
            }
            for (int i = 0; i < shootMemberCount; i++) {
                mGroupMembers.add(members.get(i));
            }
