...
public class GroupChatManager implements TIMMessageListener, UIKitMessageRevokedManager.MessageRevokeHandler {
    ...
    private void saveReceiveState(List<TIMMessage> timMessages) {
        for (TIMMessage message : timMessages) {
            ...
            if (elem instanceof TIMCustomElem) {
                ...
                if (TextUtils.equals(type, MessageInfoUtil.TRANSFER_ACCOUNT_RECEIVED)
                    ...
                    if (dataModel.getReceiveUserId().equals(TIMManager.getInstance().getLoginUser())) {
    ...
    public synchronized void sendGroupMessage(final MessageInfo message, boolean reSend, final IUIKitCallBack callBack) {
    ...
    public void createGroupChat(final GroupChatInfo chatInfo, final IUIKitCallBack callBack) {
        ...
        TIMGroupManager.getInstance().createGroup(param, new TIMValueCallBack<String>() {
            ...
            public void onSuccess(String s) {
                chatInfo.setPeer(s);
                TIMUserProfile timUserProfile = TIMFriendshipManager.getInstance().queryUserProfile(TIMManager.getInstance().getLoginUser());
                if (timUserProfile == null) {
                    ...
                    TIMFriendshipManager.getInstance().getUsersProfile(ids, false, new TIMValueCallBack<List<TIMUserProfile>>() {
                        ...
                        public void onSuccess(List<TIMUserProfile> timUserProfiles) {
                            message = timUserProfiles.get(0).getNickName().concat(ContextManager.getContext().getString(R.string.create_group));
                        }
                    });
                } else {
                    message = timUserProfile.getNickName().concat(ContextManager.getContext().getString(R.string.create_group));
                }
    ...
    private void executeMessage(TIMConversation conversation, TIMMessage msg) {
        ...
        if (msgInfo != null && mCurrentConversation != null && conversation.getPeer().endsWith(mCurrentConversation.getPeer())) {
            ...
            if (msgInfo.getMsgType() == MessageInfo.MSG_TYPE_GROUP_JOIN) {
                ...
                mCurrentChatInfo.setMemberDetails(mCurrentGroupMembers);
            } else if (msgInfo.getMsgType() == MessageInfo.MSG_TYPE_GROUP_MODIFY_NAME || msgInfo.getMsgType() == MessageInfo.MSG_TYPE_GROUP_MODIFY_NOTICE || msgInfo.getMsgType() == MessageInfo.MSG_TYPE_GROUP_MODIFY_FACE_URL) {
                ...
                if (modifyList.size() > 0) {
                    ...
                    if (modifyType == TIMGroupTipsGroupInfoType.ModifyName) {
                        ...
                        mCurrentChatInfo.setIconUrl(modifyInfo.getContent());
                    }
                }
            }
