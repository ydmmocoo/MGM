    ...
    private void init() {
        ...
        mSessionList.setOnItemLongClickListener(new AdapterView.OnItemLongClickListener() {
            ...
            public boolean onItemLongClick(AdapterView<?> adapterView, View view, int position, long id) {
                showItemPopMenu(position, mAdapter.getItem(position), view.getWidth() / 2 + view.getX(), view.getY() + view.getHeight() / 2 + DimensionUtil.dip2px(140));
                return true;
            }
        });
...
//        });
    ...
    private void showItemPopMenu(final int index, final SessionInfo sessionInfo, float locationX, float locationY) {
        ...
        for (int i = 0; i < mSessionPopActions.size(); i++) {
            ...
            if (sessionInfo.isTop()) {
                if (action.getActionName().equals(getResources().getString(R.string.chat_to_top))) {
                    action.setActionName(getResources().getString(R.string.chat_to_top_cancel));
                }
            } else {
                if (action.getActionName().equals(getResources().getString(R.string.chat_to_top_cancel))) {
                    action.setActionName(getResources().getString(R.string.chat_to_top));
                }
    ...
    public void initDefault() {
        ...
        List<PopMenuAction> sessionPopActions = new ArrayList<>();
        action = new PopMenuAction();
        action.setActionName(getResources().getString(R.string.chat_to_top));
        action.setActionClickListener(new PopActionClickListener() {
            @Override
