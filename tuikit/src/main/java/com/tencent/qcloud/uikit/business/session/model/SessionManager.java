...
public class SessionManager implements UIKitRequestHandler, TIMRefreshListener, UIKitMessageRevokedManager.MessageRevokeHandler {
    ...
    public void loadSession(IUIKitCallBack callBack) {
        ...
        final ArrayList<SessionInfo> infos = new ArrayList<>();
        ...
        Log.v("imfriend", JsonUtil.moderToString(infos));
        ...
        for (int i = 0; i < allFriend.size(); i++) {
            for (int j = 0; j < infos.size(); j++) {
                if (infos.get(j).getPeer().equals(allFriend.get(i).getIdentifier())) {
                    if (StringUtil.isNotEmpty(allFriend.get(i).getRemark()) || StringUtil.isNotEmpty(allFriend.get(i).getTimUserProfile().getNickName())) {
...
//        }
     ...
     * @return
     */
    private SessionInfo TIMConversation2SessionInfo(final TIMConversation session) {
        if (session != null) {
        ...
        if (isGroup) {
            ...
            TIMUserProfile timUserProfile = TIMFriendshipManager.getInstance().queryUserProfile(session.getPeer());
            if (timUserProfile != null) {
                ...
                TIMFriendshipManager.getInstance().getUsersProfile(ids, false, new TIMValueCallBack<List<TIMUserProfile>>() {
                    ...
                    public void onError(int i, String s) {
                        Log.d("userProfile","code "+i+"  msg " +s);
                        info.setTitle(StringUtil.phoneText(session.getPeer()));
                    }
