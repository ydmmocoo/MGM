...
public class NearbyCityPayActivity extends BaseMvpActivity<NearbyCityPayPresenter> implements NearbyCityPayContract.View {
    ...
    public static Intent newInstance(Context context, String ext) {
        Intent intent = new Intent(context, NearbyCityPayActivity.class);
    ...
    @Override
    protected NearbyCityPayPresenter createPresenter() {
        return new NearbyCityPayPresenter(this);
    }
    ...
    BroadcastReceiver broadcastReceiver = new BroadcastReceiver() {//密码支付弹窗点击指纹支付
        ...
        public void onReceive(Context context, Intent intent) {
            ...
            if (isFingerEnable) {
                mPresenter.setNearbyCityPriceBy(payType, payMap);
            } else {
    ...
    protected void initView() {
        ...
        payCallbackReceiver = PayConfig.registPayReceiver(this, new PayCallback() {
            ...
            public void payResult(PayExtModel extModel) {
                ...
                UsagePayMode type = extModel.getType();
                switch (type) {
                    case nearby_city_set_top_pay:
                        ...
                        finish();
                        break;
                    case nearby_city_pay:
                        mPresenter.requestMoney((String) extModel.getMessage(), type);
                        break;
                    default:
                }
        ...
        String price = (String) payMap.get(IntentConstants.NERBY_CITY_TOLPRICE);
        totalPriceAR = Double.parseDouble(price);
        Log.e("x3", "" + price);
        tvARPrice.setText(price);
...
//        tvARPrice.setText(totalPrice.endsWith(".0") ? totalPrice.substring(0, totalPrice.length() - 2).concat("AR") : totalPrice.concat("AR"));
    ...
    @Override
    public void responsetMoney(String money, UsagePayMode type) {
        SoundPlayUtils.play(2);//支付状态展示
        startActivity(NearbyCityPaySuccessActivity.newInstance(getCurContext(), money));
        CommonToast.toast(getString(R.string.pay_success));
        setResult(111);
        finish();
    }
    ...
    protected void onDestroy() {
        ...
        if (mPresenter.handler != null) {
            mPresenter.handler.removeCallbacks(null);
        }
        super.onDestroy();
    }
