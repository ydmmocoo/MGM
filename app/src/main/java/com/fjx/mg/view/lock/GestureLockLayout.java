...
public class GestureLockLayout extends RelativeLayout {
    ...
    //手指触摸是，path颜色
    private int mFingerTouchColor = Color.parseColor("#FBB03B");
    //手指抬起时,密码匹配path颜色
    private int mFingerUpMatchedColor = Color.parseColor("#FBB03B");
    //手指抬起时,密码不匹配path颜色
...
//                    mAnswerList.add(integer-1);
    ...
    private boolean checkAnswer() {
        ...
        Log.d("checkAnswer", "mAnswerList:" + JsonUtil.moderToString(mAnswerList));
        Log.d("checkAnswer", "mChooseList:" + JsonUtil.moderToString(mChooseList));
        Log.d("checkAnswer", "answerSting:" + answerSting);

        if (!TextUtils.isEmpty(answerSting)) {
            String chooseString = "";
            for (int i = 0; i < mChooseList.size(); i++) {
                chooseString = chooseString.concat(String.valueOf(mChooseList.get(i)));
            }
            return TextUtils.equals(answerSting, StringUtil.getPassword(chooseString));
        }
    ...
    public void setAnswerSting(String answerSting) {
