...
public class FriendsMomentsAdapter extends BaseAdapter<MomentsListBean> {

    private boolean isPraised = false;
    private FriendsMomentsPresenter mPresenter;
    private SnsPopupWindow snsPopupWindow;

    void setCirclePresenter(FriendsMomentsPresenter mPresenter) {
        this.mPresenter = mPresenter;
    }

    public FriendsMomentsAdapter() {
...
//        helper.setText(R.id.tvContent, momentsInfo.getContent());
            ...
            if (StringUtil.isNotEmpty(momentsInfo.getDistanceUnit())) {
...
//        tagAdapter.setList(tags.size() > 3 ? tags.subList(0, 3) : tags);//标签列表数据
            ...
            imageAdapter.setList(momentsInfo.getThumUrls());
    ...
    private void initTags(BaseViewHolder helper, MomentsListBean item) {
        ...
        tagRecycler.setAdapter(tagAdapter);
        if (item.getUserInfo().getTags().isEmpty()) {
    ...
    private void initTextContent(BaseViewHolder helper, final String content, final MomentsListBean.MomentsInfoBean.ShareInfoBean shareInfo) {
        final ExpandTextView tvContent = helper.getView(R.id.tvContent);
...
//            helper.setVisible(R.id.llShareContainer, true);
...
//            helper.setVisible(R.id.llShareContainer, false);
...
//                dialog.show();
                View contentView = LayoutInflater.from(view.getContext()).inflate(R.layout.dialog_copy_tips, null);
                ...
                popupWindow.showAsDropDown(view, width / 3, heght / 3);
    ...
    /*protected void saveImageFile(String imageUri) {
        ...
        if (mkFlag) {
            ...
            if (success)
                CommonToast.toast(getContext().getResources().getString(R.string.picture_save_success));
        }
    }*/
}
