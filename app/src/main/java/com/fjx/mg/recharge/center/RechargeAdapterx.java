...
public class RechargeAdapterx extends BaseAdapter<RechargeModel> {
    ...
    protected void convert(BaseViewHolder helper, RechargeModel item) {
        ...
        if (isPhoneCharge) {
            if (isPhone) {
                ...
                tvPrice.setText("售价：" + item.getPrice() + item.getUnit());
            } else {
                tvContent.setText(item.getPackageX() + item.getCunit());
                tvPrice.setText("售价：" + item.getPrice() + item.getFunit());
            }
        } else {
            tvContent.setText(item.getPackageX());
            tvPrice.setText("售价：" + item.getPrice() + "AR");
        }
